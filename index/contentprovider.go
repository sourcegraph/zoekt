...
func (p *contentProvider) fillMatches(ms []*candidateMatch, numContextLines int, language string, opts *zoekt.SearchOptions) []zoekt.LineMatch {
	...
	// Otherwise, we return a single line containing the filematch index.
	...
	for _, m := range ms {
		res.LineFragments = append(res.LineFragments, zoekt.LineFragmentMatch{
...
// Note: the byte slices may be backed by mmapped data, so before being
// returned by the API it needs to be copied.
func (p *contentProvider) fillChunkMatches(ms []*candidateMatch, numContextLines int, language string, opts *zoekt.SearchOptions) []zoekt.ChunkMatch {
	var filenameMatches []*candidateMatch
	contentMatches := make([]*candidateMatch, 0, len(ms))
	...
	// Otherwise, we return a single chunk representing the filename index.
	lineScore, _ := p.scoreLine(filenameMatches, language, -1 /* must pass -1 for filenames */, opts)
	fileName := p.id.fileName(p.idx)
	ranges := make([]zoekt.Range, 0, len(ms))
	for _, m := range ms {
		ranges = append(ranges, zoekt.Range{
			Start: zoekt.Location{
...
func (p *contentProvider) fillContentMatches(ms []*candidateMatch, numContextLines int, language string, opts *zoekt.SearchOptions) []zoekt.LineMatch {
	var result []zoekt.LineMatch
	for len(ms) > 0 {
		m := ms[0]
		...
		// taking lines until we pass the last index
		...
		for i, m := range lineCands {
			fragment := zoekt.LineFragmentMatch{
				Offset:      m.byteOffset,
...
func (p *contentProvider) fillContentChunkMatches(ms []*candidateMatch, numContextLines int, language string, opts *zoekt.SearchOptions) []zoekt.ChunkMatch {
	data := p.data(false)
	...
	chunkMatches := make([]zoekt.ChunkMatch, 0, len(chunks))
	for _, chunk := range chunks {
		ranges := make([]zoekt.Range, 0, len(chunk.candidates))
		...
		chunkMatches = append(chunkMatches, zoekt.ChunkMatch{
...
func (p *contentProvider) findSymbol(cm *candidateMatch) (DocumentSection, *zoekt.Symbol, bool) {
	if cm.fileName {
...
type matchScoreSlice []zoekt.LineMatch
...
type chunkMatchScoreSlice []zoekt.ChunkMatch
...
type fileMatchesByScore []zoekt.FileMatch
...
func sortMatchesByScore(ms []zoekt.LineMatch) {
	sort.Sort(matchScoreSlice(ms))
}

func sortChunkMatchesByScore(ms []zoekt.ChunkMatch) {
...
func boostNovelExtension(ms []zoekt.FileMatch, boostOffset int, minScoreRatio float64) {
