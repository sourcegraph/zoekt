...
func TestRankGeneratedTestKeepsGeneratedPenalty(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "indexed"},
		{name: "too many trigrams", trigramMax: 1, wantSkip: SkipReasonTooManyTrigrams},
	} {
		t.Run(tc.name, func(t *testing.T) {
			b, err := NewBuilder(Options{
				IndexDir:              t.TempDir(),
				DisableCTags:          true,
				TrigramMax:            tc.trigramMax,
			...
			doc := b.todo[0]
			if doc.SkipReason != tc.wantSkip {
				t.Fatalf("skip reason = %v, want %v", doc.SkipReason, tc.wantSkip)
			}
			if tc.wantSkip != SkipReasonNone && doc.Content != nil {
				t.Fatal("skipped content was not discarded")
			}
			if doc.Category != FileCategoryTest {
				t.Fatalf("category = %v, want test", doc.Category)
			}
			if got := rank(doc, 0)[1:4]; !reflect.DeepEqual(got, []float64{1, 1, 1}) {
				t.Fatalf("generated/vendor/test penalties = %v, want [1 1 1]", got)
			}
			if err := b.Finish(); err != nil {
				t.Fatal(err)
			}
