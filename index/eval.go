...
// otherwise.
func (d *indexData) simplifyMultiRepo(q query.Q, predicate func(*zoekt.Repository) bool) query.Q {
	count := 0
...
func (d *indexData) simplify(in query.Q) query.Q {
	eval := query.Map(in, func(q query.Q) query.Q {
		...
		case query.RawConfig:
			return d.simplifyMultiRepo(q, func(repo *zoekt.Repository) bool { return uint8(r)&encodeRawConfig(repo.RawConfig) == uint8(r) })
		case *query.RepoIDs:
			return d.simplifyMultiRepo(q, func(repo *zoekt.Repository) bool {
...
// into a single index. Otherwise, overlapping matches will be removed,
...
func (d *indexData) List(ctx context.Context, q query.Q, opts *zoekt.ListOptions) (rl *zoekt.RepoList, err error) {
	...
	if c, ok := q.(*query.Const); ok {
		...
		include = func(rle *zoekt.RepoListEntry) bool {
			return true
		}
	} else {
		sr, err := d.Search(ctx, q, &zoekt.SearchOptions{
	...
	case zoekt.RepoListFieldRepos:
		l.Repos = make([]*zoekt.RepoListEntry, 0, len(d.repoListEntry))
	case zoekt.RepoListFieldReposMap:
		l.ReposMap = make(zoekt.ReposMap, len(d.repoListEntry))
	}
	...
	for i := range d.repoListEntry {
		...
		case zoekt.RepoListFieldRepos:
			l.Repos = append(l.Repos, rle)
		case zoekt.RepoListFieldReposMap:
			l.ReposMap[rle.Repository.ID] = zoekt.MinimalRepoListEntry{
