...
func parseSymbols(todo []*Document, languageMap ctags.LanguageMap, parserBins ctags.ParserBinMap) error {
	monitor := newMonitor()
	defer monitor.Stop()
...
func overlaps(symOffsets []DocumentSection, start, end uint32) int {
	i := 0
...
func (t *tagsToSections) Convert(content []byte, tags []*ctags.Entry) ([]DocumentSection, []*zoekt.Symbol, error) {
	...
	for _, t := range tags {
		...
		symOffsets = slices.Insert(symOffsets, i, DocumentSection{
			Start: start,
			End:   endSym,
		})
