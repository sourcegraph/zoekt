...
func clearScores(r *zoekt.SearchResult) {
	for i := range r.Files {
		r.Files[i].Score = 0.0
	...
	t.Run("ChunkMatches", func(t *testing.T) {
		...
		want := []zoekt.FileMatch{{
			...
			ChunkMatches: []zoekt.ChunkMatch{{
				...
				Ranges: []zoekt.Range{{
					Start: zoekt.Location{ByteOffset: 8, LineNumber: 2, Column: 3},
					End:   zoekt.Location{ByteOffset: 11, LineNumber: 2, Column: 6},
				}},
			}},
		}}
...
func TestSearchStats(t *testing.T) {
	...
	t.Run("LineMatches", func(t *testing.T) {
		sres, err := searcher.Search(ctx, andQuery, &zoekt.SearchOptions{})
		if err != nil {
			t.Fatal(err)
		}
		matches := sres.Files
	...
	t.Run("Stats", func(t *testing.T) {
		...
		for _, tc := range cases {
			t.Run(tc.Name, func(t *testing.T) {
				...
				if diff := cmp.Diff(tc.Want, sres.Stats, cmpopts.IgnoreFields(zoekt.Stats{}, "MatchTreeConstruction", "MatchTreeSearch")); diff != "" {
...
func TestFileSearch(t *testing.T) {
	...
	t.Run("ChunkMatches", func(t *testing.T) {
		...
		want := zoekt.ChunkMatch{
			Content:      []byte("banana"),
			ContentStart: zoekt.Location{ByteOffset: 0, LineNumber: 1, Column: 1},
			Ranges: []zoekt.Range{{
				Start: zoekt.Location{ByteOffset: 1, LineNumber: 1, Column: 2},
	...
	t.Run("FileNameSet", func(t *testing.T) {
		...
		want := zoekt.ChunkMatch{
			...
			Ranges: []zoekt.Range{{
				...
				End:   zoekt.Location{ByteOffset: 6, LineNumber: 1, Column: 7},
			}},
			FileName: true,
		}
...
func TestSearchBM25MatchScores(t *testing.T) {
	...
	t.Run("LineMatches", func(t *testing.T) {
		...
		sres, err := searcher.Search(ctx, q, &zoekt.SearchOptions{UseBM25Scoring: true})
		if err != nil {
			t.Fatal(err)
		}
		matches := sres.Files
		if len(matches) != 1 {
			t.Fatalf("want 1 file index, got %d", len(matches))
		}
	...
	t.Run("ChunkMatches", func(t *testing.T) {
		...
		sres, err := searcher.Search(ctx, q, &zoekt.SearchOptions{UseBM25Scoring: true, ChunkMatches: true, NumContextLines: 1})
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("ChunkMatches with symbols", func(t *testing.T) {
		...
		matches := sres.Files
		if len(matches) != 2 {
			t.Fatalf("want 2 file index, got %d", len(matches))
		}
...
func TestRegexp(t *testing.T) {
	...
	t.Run("ChunkMatches", func(t *testing.T) {
		...
		want := zoekt.ChunkMatch{
			...
			ContentStart: zoekt.Location{ByteOffset: 0, LineNumber: 1, Column: 1},
			Ranges: []zoekt.Range{{
				Start: zoekt.Location{ByteOffset: 3, LineNumber: 1, Column: 4},
				End:   zoekt.Location{ByteOffset: 14, LineNumber: 1, Column: 15},
			}},
		}
...
func TestRegexpCaseSensitive(t *testing.T) {
	...
	t.Run("LineMatches", func(t *testing.T) {
		...
		if len(res.Files) != 1 {
			t.Fatalf("got %v, want one index", res.Files)
		}
	})
...
func TestSymbolRank(t *testing.T) {
	...
	t.Run("LineMatches", func(t *testing.T) {
		...
		if res.Files[0].FileName != "f2" {
			t.Errorf("got %#v, want 'f2' as top index", res.Files[0])
		}
	})
...
func TestListRepos(t *testing.T) {
	...
	t.Run("default and minimal fallback", func(t *testing.T) {
		...
		for _, opts := range []*zoekt.ListOptions{
			nil,
			{},
			{Field: zoekt.RepoListFieldRepos},
			{Field: zoekt.RepoListFieldReposMap},
		} {
	...
	t.Run("minimal", func(t *testing.T) {
		...
		q := &query.Repo{Regexp: regexp.MustCompile("epo")}
		res, err := searcher.List(context.Background(), q, &zoekt.ListOptions{Field: zoekt.RepoListFieldReposMap})
		if err != nil {
			t.Fatalf("List(%v): %v", q, err)
		}
...
func TestSubRepo(t *testing.T) {
	subRepos := map[string]*zoekt.Repository{
		"sub": {
			Name:                 "sub-name",
...
func TestSearchEither(t *testing.T) {
	...
	t.Run("LineMatches", func(t *testing.T) {
		...
		if len(sres.Files) != 1 {
			t.Fatalf("got %v, wanted 1 index", sres.Files)
		}
...
func TestUnicodeExactMatch(t *testing.T) {
	...
	t.Run("LineMatches", func(t *testing.T) {
		if res := searchForTest(t, b, &query.Substring{Pattern: needle, CaseSensitive: true}); len(res.Files) != 1 {
			t.Fatalf("case sensitive: got %v, wanted 1 index", res.Files)
		}
	})
...
func TestUnicodeCoverContent(t *testing.T) {
	...
	t.Run("LineMatches", func(t *testing.T) {
		if res := searchForTest(t, b, &query.Substring{Pattern: "NÉÉDLÉ", CaseSensitive: true}); len(res.Files) != 0 {
			t.Fatalf("case sensitive: got %v, wanted 0 index", res.Files)
		}

		res := searchForTest(t, b, &query.Substring{Pattern: "NÉÉDLÉ"})
		if len(res.Files) != 1 {
			t.Fatalf("case insensitive: got %v, wanted 1 index", res.Files)
		}
...
func TestUnicodeNonCoverContent(t *testing.T) {
	...
	t.Run("LineMatches", func(t *testing.T) {
		...
		if len(res.Files) != 1 {
			t.Fatalf("got %v, wanted 1 index", res.Files)
		}
...
func TestNoCollectRegexpSubstring(t *testing.T) {
	...
	t.Run("LineMatches", func(t *testing.T) {
		...
		if f := res.Files[0]; len(f.LineMatches) != 1 {
			t.Fatalf("got line matches %v, want 1 line index", printLineMatches(f.LineMatches))
		}
	})
...
func printLineMatches(ms []zoekt.LineMatch) string {
	var ss []string
	for _, m := range ms {
...
func TestHitIterTerminate(t *testing.T) {
	...
	// def for the distance iteration. There is no index, so this
...
func TestStats(t *testing.T) {
	ignored := []cmp.Option{
		cmpopts.EquateEmpty(),
		cmpopts.IgnoreFields(zoekt.RepoListEntry{}, "Repository"),
		cmpopts.IgnoreFields(zoekt.RepoListEntry{}, "IndexMetadata"),
		cmpopts.IgnoreFields(zoekt.RepoStats{}, "IndexBytes"),
	}
	...
	t.Run("compound shard with empty repos", func(t *testing.T) {
		...
		entryEmpty := zoekt.RepoListEntry{Stats: zoekt.RepoStats{
			Shards:       1,
			Documents:    0,
			ContentBytes: 0,
		}}
		entryNonEmpty := zoekt.RepoListEntry{Stats: zoekt.RepoStats{
			Shards:       1,
			Documents:    1,
			ContentBytes: 14,
		}}
...
func TestWordSearch(t *testing.T) {
	...
	t.Run("ChunkMatches", func(t *testing.T) {
		...
		want := zoekt.ChunkMatch{
			...
			ContentStart: zoekt.Location{ByteOffset: 0, LineNumber: 1, Column: 1},
			Ranges: []zoekt.Range{{
				Start: zoekt.Location{ByteOffset: 7, LineNumber: 1, Column: 8},
				End:   zoekt.Location{ByteOffset: 10, LineNumber: 1, Column: 11},
			}},
		}
...
func BenchmarkScoreChunkMatches(b *testing.B) {
	...
	b.Run("score large ChunkMatch", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			sres, err := searcher.Search(ctx, q, &zoekt.SearchOptions{ChunkMatches: true, NumContextLines: 1})
			if err != nil {
				b.Fatal(err)
			}
			...
			matches := sres.Files
			if len(matches) == 0 {
				b.Fatalf("want file index, got none")
			}
		}
	})
}
