...
func (d *symbolData) data(i uint32) *zoekt.Symbol {
...
func (d *indexData) calculateStats() error {
	d.repoListEntry = make([]zoekt.RepoListEntry, 0, len(d.repoMetaData))
	...
	for repoID, md := range d.repoMetaData {
		...
		d.repoListEntry = append(d.repoListEntry, zoekt.RepoListEntry{
			Repository:    md,
