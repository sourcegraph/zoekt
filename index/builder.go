...
func (o *Options) shardName(n int) string {
	return o.shardNameVersion(IndexFormatVersion, n)
}
...
func (o *Options) IndexState() (IndexState, string) {
	...
	repos, index, err := ReadMetadataPathAlive(fn)
...
func (o *Options) FindRepositoryMetadata() (repository *zoekt.Repository, metadata *zoekt.IndexMetadata, ok bool, err error) {
	...
	repositories, metadata, err := ReadMetadataPathAlive(shard)
	if err != nil {
...
func (b *Builder) Finish() error {
	...
	if b.opts.IsDelta {
		...
		for _, shard := range oldShards {
			repositories, _, err := ReadMetadataPathAlive(shard)
			if err != nil {
