...
func GetLanguageExtensions(language string) []string {
	...
	for ext, lang := range sgExtraLangsForExts { // Map is tiny, so linear lookup is fine
		if language == lang {
	...
	return slices.DeleteFunc(exts, func(ext string) bool {
...
//   - Some languages are not supported by enry yet (e.g. Magik)
func getLanguagesByExtension(path string) (candidates []string, isLikelyBinaryFile bool) {
	// Lowercase extension before lookups to match enry's behavior.
	...
	candidates = enry.GetLanguagesByExtension(path, nil, nil)
	if extra, ok := sgExtraLangsForExts[ext]; ok {
...
var sgExtraLangsForExts = map[string]string{
	".c": "C++",
	// NOTE: Downstream code does linear lookups on this map, so
	// be careful if you're adding lots of entries here.
}

var sgExtraContentHeuristics = map[string]*enrydata.Heuristics{
	".c": enrydata.ContentHeuristics[".h"],
}

// overrideAmbiguousExtensionsMap represents extensions which are ambiguous according to
// enry but not for Sourcegraph.
var overrideAmbiguousExtensionsMap = map[string]string{
	...
	// as it is niche.
	".fs": "F#",
	// Ignoring the uncommon usage of '.html' for Ecmarkup.
	".html": "HTML",
	...
	".yaml": "YAML",
	".yml":  "YAML",
	// The PR adding Pkl support also listed another language called Pickle in
	// its heuristics, but doesn't have any real support for it. Just ignore
	// it.
	// https://github.com/github-linguist/linguist/pull/6730/files#diff-c2d2d7946540ab501a5ef7a7f54a57c530d8da599e41c2beb0fd2f5635d2fd50R539
	".pkl": "Pkl",
}

// unsupportedByEnryExtensionToNameMap contains extension->name mappings
// for languages not tracked by go-enry.
...
// unsupportedByEnryNameToExtensionMap contains language->extension mappings
// for languages not tracked by go-enry.
