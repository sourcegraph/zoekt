...
func findNonUTF8StringFields(m proto.Message) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("iterating over proto message: %w", err)
	}
...
func envMustGetBool(key string, defaultValue bool) bool {
	...
	if err != nil {
		panic(fmt.Sprintf("Failed to parse enviroment variable %q as valid boolean. Got %q. Err: %s", key, rawValue, err))
	}

	return value
}

func envMustGetBytes(key string, defaultByteSize string) uint64 {
	defaultByteSizeValue, err := humanize.ParseBytes(defaultByteSize)
	if err != nil {
		panic(fmt.Sprintf("Failed to parse default byte size %q as valid byte size. Err: %s", defaultByteSize, err))
	}

	rawValue, ok := os.LookupEnv(key)
	if !ok {
		return defaultByteSizeValue
	}

	value, err := humanize.ParseBytes(rawValue)
	if err != nil {
		panic(fmt.Sprintf("Failed to parse enviroment variable %q as valid byte size. Got %q. Err: %s", key, rawValue, err))
	}
