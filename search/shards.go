...
var branchesReposBlooms = sync.Pool{
	New: func() any { return new(branchesReposBloom) },
}
...
func newBranchesReposSelector(branches []query.BranchRepos, cardinality uint64) *branchesReposSelector {
	return &branchesReposSelector{
		branches:    branches,
		cardinality: cardinality,
		settled:     !branchesReposCanBuildBloom(cardinality),
	}
}
...
func (s *branchesReposSelector) maybeBuildBloom(remaining []*rankedShard) *branchesReposBloom {
	...
	bloom := branchesReposBlooms.Get().(*branchesReposBloom)
...
// selectBranchesRepos allocates the fixed filter only after observed misses
// justify it.
func selectBranchesRepos(shards []*rankedShard, branches []query.BranchRepos, cardinality uint64, setSize int) ([]*rankedShard, bool) {
	...
	for shardIndex, shard := range shards {
		...
		for _, repo := range shard.repos {
			matched := false
			if bloom == nil || bloom.mayContain(repo.ID) {
				matched = branches[selector.preferred].Repos.Contains(repo.ID)
				if !matched {
					for i, branch := range branches {
						if i == selector.preferred {
							continue
						}
						if branch.Repos.Contains(repo.ID) {
							selector.preferred = i
							matched = true
							break
						}
					}
					if !matched {
						selector.missProbes += uint64(len(branches))
					}
				}
			}
			any = any || matched
		...
		if !selector.settled && selector.missProbes >= branchesReposMinimumMissProbes && bloom == nil {
			bloom = selector.maybeBuildBloom(shards[shardIndex+1:])
		}
		if any {
			filtered = append(filtered, shard)
	...
	if bloom != nil {
		clear(bloom.bits[:])
		branchesReposBlooms.Put(bloom)
	}
	return filtered, filteredAll
}

// selectBranchesReposDirect avoids adaptive filter bookkeeping when the fixed
// filter would be saturated.
func selectBranchesReposDirect(shards []*rankedShard, branches []query.BranchRepos, setSize int) ([]*rankedShard, bool) {
	preferred := 0
	filtered := make([]*rankedShard, 0, setSize)
	filteredAll := true
	for _, shard := range shards {
		...
		any, all := false, true
		for _, repo := range shard.repos {
			matched := branches[preferred].Repos.Contains(repo.ID)
			if !matched {
				for i, branch := range branches {
					if i != preferred && branch.Repos.Contains(repo.ID) {
						preferred = i
						matched = true
						break
					}
...
func doSelectRepoSet(shards []*rankedShard, and *query.And) ([]*rankedShard, query.Q) {
	...
	for i, c := range and.Children {
		...
		case *query.BranchesRepos:
			if len(setQuery.List) == 0 {
				hasRepos = hasReposForPredicate(func(*zoekt.Repository) bool { return false })
				break
			}
		...
		if branchesForSelector != nil {
			if branchesReposCanBuildBloom(branchesCardinality) {
				filtered, filteredAll = selectBranchesRepos(shards, branchesForSelector, branchesCardinality, setSize)
			} else {
				filtered, filteredAll = selectBranchesReposDirect(shards, branchesForSelector, setSize)
			}
		} else {
			filtered = make([]*rankedShard, 0, setSize)
			filteredAll = true
