...
func setTombstone(shardPath string, repoID uint32, tombstone bool) error {
	...
	dest := shardPath + ".meta"
	temp, err := JsonMarshalTemp(repos, filepath.Dir(dest), filepath.Base(dest)+".*.tmp")
	if err != nil {
		return err
	}
...
// JsonMarshalTemp writes the JSON encoding of v to a temporary file
...
func JsonMarshalTemp(v interface{}, dir, pattern string) (tempFile string, err error) {
