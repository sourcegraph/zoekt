...
func (s *loggedSearcher) log(ctx context.Context, q query.Q, opts *zoekt.SearchOptions, st *zoekt.Stats, err error) {
	...
	logger.Info("search",
		sglog.Int64("st.ContentBytesLoaded", st.ContentBytesLoaded),
		sglog.Int64("st.IndexBytesLoaded", st.IndexBytesLoaded),
		sglog.Int("st.Crashes", st.Crashes),
		sglog.Duration("st.Duration", st.Duration),
		sglog.Int("st.FileCount", st.FileCount),
		sglog.Int("st.ShardFilesConsidered", st.ShardFilesConsidered),
		sglog.Int("st.FilesConsidered", st.FilesConsidered),
		sglog.Int("st.FilesLoaded", st.FilesLoaded),
		sglog.Int("st.FilesSkipped", st.FilesSkipped),
		sglog.Int("st.ShardsScanned", st.ShardsScanned),
		sglog.Int("st.ShardsSkipped", st.ShardsSkipped),
		sglog.Int("st.ShardsSkippedFilter", st.ShardsSkippedFilter),
		sglog.Int("st.MatchCount", st.MatchCount),
		sglog.Int("st.NgramMatches", st.NgramMatches),
		sglog.Duration("st.Wait", st.Wait),
	)
}
