...
func TestIndex(t *testing.T) {
	cases := []struct {
		...
		want: []string{
			...
			"git -C $TMPDIR/test%2Frepo.git config --replace-all http.extraHeader X-Sourcegraph-Actor-UID: internal",
			"git -C $TMPDIR/test%2Frepo.git config --add http.extraHeader X-Sourcegraph-Tenant-ID: 42",
			"git -C $TMPDIR/test%2Frepo.git -c protocol.version=2 fetch --depth=1 --no-tags --atomic --no-auto-maintenance --filter=blob:limit=1048577 http://api.test/.internal/git/test/repo deadbeef",
		...
		want: []string{
			...
			"git -C $TMPDIR/test%2Frepo.git config --replace-all http.extraHeader X-Sourcegraph-Actor-UID: internal",
			"git -C $TMPDIR/test%2Frepo.git config --add http.extraHeader X-Sourcegraph-Tenant-ID: 1",
			"git -C $TMPDIR/test%2Frepo.git -c protocol.version=2 fetch --depth=1 --no-tags --atomic --no-auto-maintenance http://api.test/.internal/git/test/repo deadbeef feebdaed",
			"git -C $TMPDIR/test%2Frepo.git update-ref HEAD deadbeef",
		...
		want: []string{
			...
			"git -C $TMPDIR/test%2Frepo.git config --replace-all http.extraHeader X-Sourcegraph-Actor-UID: internal",
			"git -C $TMPDIR/test%2Frepo.git config --add http.extraHeader X-Sourcegraph-Tenant-ID: 1",
			"git -C $TMPDIR/test%2Frepo.git -c protocol.version=2 fetch --depth=1 --no-tags --atomic --no-auto-maintenance http://api.test/.internal/git/test/repo deadbeef feebdaed 12345678 oldhead olddev oldrelease",
			"git -C $TMPDIR/test%2Frepo.git update-ref HEAD deadbeef",
			"git -C $TMPDIR/test%2Frepo.git update-ref refs/heads/dev feebdaed",
